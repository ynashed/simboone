...
class SimIonization(torch.nn.Module):
    def __init__(self, test_case=0, std_in=0.1):
        ...
        if test_case == 0:
            self._v_alpha = torch.tensor(self._alpha, requires_grad=False)
            self._v_beta = torch.tensor(self._beta, requires_grad=False)
        elif test_case == 1:
            self._v_beta = torch.nn.Parameter(torch.rand(()))
            self._v_alpha = torch.nn.Parameter(torch.rand(()))
        else:            
            self._v_alpha = torch.nn.Parameter(torch.normal(mean=self._alpha, std=std_in, size=()))
            self._v_beta = torch.nn.Parameter(torch.normal(mean=self._beta, std=std_in, size=()))            

        self._v_efield = torch.nn.Parameter(torch.rand(()))
        self._v_lifetime = torch.nn.Parameter(torch.normal(mean=self._lifetime, std=std_in, size=()))   
